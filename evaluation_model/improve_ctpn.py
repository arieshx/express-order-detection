...
"""
从大数据集中筛选出目前ctpn检测不对的数据,对模型进行训练,目前主要的问题是漏框
"""
...
c_det_frcnn.connect("tcp://192.168.1.115:18005")  # kuaidi

save_dir = '/media/haoxin/A1/all_data_result/ctpn_result'
...
list_xmls_path = glob.glob(os.path.join(xmls_dir, '*.xml'))[0:]
...
for odx, xml_path in enumerate(list_xmls_path):
    ...
    list_match_local, list_poss_local, list_match_pair, list_poss_pair, flag = evaluation.is_qualified_two_img(image,
                                                                                                               list_local_frame,
                                                                                                               list_gt_frame)
