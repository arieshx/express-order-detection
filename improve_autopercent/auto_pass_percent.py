...
cate_right_num = {
        ...
        'rcProvName': 0,
        'rcTownName': 0,
        'receiveCustomerAddress': 0,
        'receiveCustomerName': 0,
        'receiveCustomerPhone': 0,
        'only_prov': 0,
        'prov_city': 0,
        'prov_city_dist': 0,
        'prov_city_dist_town': 0,
        'prov_city_dist_town_detail': 0,
...
jiexi_right_num = 0
jiexi_error_num = 0
jiexi_error_image_path = []
for idx, inst in enumerate(data[200: ]):
    image_path = inst['url']
    uid = inst['uid']
    if '_163.jpg' not in image_path: continue

    # care_list = list()
    # with open('./data/jiexi_error_path.txt', 'r') as f:
    #     lines = f.readlines()
    ...
    for name in sorted(detect_res.keys()):
        if type(detect_res[name]) == dict:
            if name == 'receiveCustomerAddress':
                print name, detect_res[name]['text'], detect_res[name]['AddrRes']['msg']
            else:
    ...
    for name in deppon_res.keys():
        ...
        if name not in ['goodsName']: continue
    ...
    diff_key = list()
    address_pass = dict()
    if deppon_res['receiveCustomerAddress'] == -1:
        for name in ['rcProvName', 'rcCityName', 'rcDistName', 'rcTownName', 'receiveCustomerAddress']:
            cate_right_num[name] += 0
            address_pass[name] = 0
            diff_key.append(name)
    else:
        assert type(detect_res['receiveCustomerAddress']) == dict, 'address error' + str(
            detect_res['receiveCustomerAddress'])
        if detect_res['receiveCustomerAddress']['AddrRes']['value'] and detect_res['receiveCustomerAddress']['is_valid_pca'] :
            jiexi_num += 1
            for name in ['rcProvName', 'rcCityName', 'rcDistName', 'rcTownName', 'receiveCustomerAddress']:
                if name =='rcProvName':
                    if deppon_res[name] in [detect_res[name]['text'], detect_res[name]['text'][:-1]]:
                ...
                if detect_res[name]['text'] == deppon_res[name]:
                    ...
                    address_pass[name] = 0
                    diff_key.append(name)
            if deppon_res[name] in [detect_res[name]['text'], detect_res[name]['text'][:-1]] and detect_res['rcCityName']['text'] == deppon_res['rcCityName'] and detect_res['rcDistName']['text'] == deppon_res['rcDistName']:
                jiexi_right_num+=1
            else:
                jiexi_error_num+=1
                jiexi_error_image_path.append(image_path)
                txt_path = './data/' + os.path.basename(image_path)[:-4]+'.txt'
                with open(txt_path, 'w') as f:
                    f.write(('detect_prov:'+detect_res['rcProvName']['text']+'\tdeppon_prov:'+deppon_res['rcProvName']+'\n').encode('utf-8'))
                    f.write(('detect_city:' + detect_res['rcCityName']['text'] + '\tdeppon_city:' + deppon_res[
                        'rcCityName'] + '\n').encode('utf-8'))
                    f.write(('detect_dist:' + detect_res['rcDistName']['text'] + '\tdeppon_city:' + deppon_res[
                        'rcDistName']).encode('utf-8'))
                with open('./data/jiexi_error_path.txt', 'a') as f:
                    f.write(image_path + '\n')
        else:
            for name in ['rcProvName', 'rcCityName', 'rcDistName', 'rcTownName', 'receiveCustomerAddress']:
                cate_right_num[name] += 0
    ...
    tmp = list(address_pass.values())
    if address_pass['rcProvName']:
        cate_right_num['only_prov'] += 1
    if address_pass['rcProvName'] and address_pass['rcCityName']:
        cate_right_num['prov_city'] += 1
    if address_pass['rcProvName'] and address_pass['rcCityName'] and address_pass['rcDistName']:
        cate_right_num['prov_city_dist'] += 1
    if address_pass['rcProvName'] and address_pass['rcCityName'] and address_pass['rcDistName'] and address_pass['rcTownName']:
        cate_right_num['prov_city_dist_town'] += 1
    if address_pass['rcProvName'] and address_pass['rcCityName'] and address_pass['rcDistName'] and address_pass['rcTownName'] and address_pass['receiveCustomerAddress']:
        cate_right_num['prov_city_dist_town_detail'] += 1
    ...
    for name in deppon_res.keys():
        ...
        # 地址判断是否通过在for循环外写
        if name in ['rcProvName', 'rcCityName', 'rcDistName', 'rcTownName', 'receiveCustomerAddress']: continue
...
for _ in cate_result:
    print(_)
print('jiexi num:', jiexi_num)
print('jiexi_right:', jiexi_right_num)
print('jiexi_error:', jiexi_error_num)
jiexi_error_dict['jiexi_prov_error'] = (jiexi_num - cate_right_num['rcProvName'])*1./jiexi_num
jiexi_error_dict['jiexi_city_error'] = (jiexi_num - cate_right_num['rcCityName'])*1./jiexi_num
jiexi_error_dict['jiexi_dist_error'] = (jiexi_num - cate_right_num['rcDistName'])*1./jiexi_num
for _ in jiexi_error_dict.keys():
    print(_, jiexi_error_dict[_])
