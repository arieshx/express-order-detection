...
生成整个模型回归目标的真实答案用来参与损失函数的计算来迭代模型
    1.采用hard negtive，这一训练方法实现在RCNN网络中，我没有好好看过那一块，我的实现可能与论文原作有差异，但是希望可以work
    2.第一次随机取正负样本，控制比例比如1:3， 训练一次模型
    3.用训练了一次的模型预测第一次没有参与训练的负样本，将预测值较大的高于某个阈值的（比如0.5）的负样本记下来，加入到第一次的训练数据中，继续训练，比如继续训练n次，或者制定一个出口，
"""
import numpy as np
...
tmp_labels = None
tmp_all_bg_index = None
tmp_v_target = None
tmp_o_target = None
tmp_j_index2 = None
tmp_k_index2 = None
tmp_inds_inside = None
...
def anchor_target_layer(rpn_cls_score, rpn_cls_prob, im_name, gt_boxes_large, gt_ishard, dontcare_areas, im_info, _feat_stride = [16,], anchor_scales = [16,]):
    ...
    if flag_first:  # 如果是第一次见到这个图片，就要重新生成所有tmp对象
        ...
        if cfg.TRAIN.PRECLUDE_HARD_SAMPLES and gt_ishard is not None and gt_ishard.shape[0] > 0 and 0:
        ...
        if DEBUG:
            print('加载进来的第一次的labels的负样本的数量：'+str(len(np.where(labels == 0)[0])))
            print('属于难负样本不属于第一次样本的数量：' + str(len(np.where(new_hard_index == True)[0])))
