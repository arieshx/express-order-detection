...
def anchor_target_layer(rpn_cls_score, gt_boxes, gt_ishard, dontcare_areas, im_info, _feat_stride = [16,], anchor_scales = [16,]):
    ...
    max_overlaps = overlaps[np.arange(len(inds_inside)), argmax_overlaps]   # 假如在内部的anchor有900个 ，(900,), 表示的是每一个anchor最大的overlaps值
    ...
    gt_max_overlaps = overlaps[gt_argmax_overlaps,
                               np.arange(overlaps.shape[1])]  #  比如有3个gt 那么就得到(3,),表示的是上一步找到的与gt的overlap最大的3个anchor的overlap值
...
if __name__ == '__main__':
    rpn_cls_score = np.zeros((1,30,40,20))
    gt_boxes = np.array([[100,100,300,160,1],[200,230,300,300,1],[200,30,300,90,1 ]])
    gt_ishard = np.zeros((3,))
    dontcare_areas = None
    im_info = np.array([[200,200,3]], dtype=np.float32)
    anchor_target_layer(rpn_cls_score, gt_boxes, gt_ishard, dontcare_areas, im_info)